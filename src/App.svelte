<script lang="ts">
	...
	async function getData(): Promise<ResponseData> {
		...
		var trades: Array<Trade> = getCachedData<Array<Trade>>(
			"trades",
			cacheLifeTime,
			[],
		);
		var info = getCachedData<Array<ItemInfo>>(
			"extendedInfo",
			cacheLifeTime,
			[],
		);
		...
		if (data.trades.length < 1) {
			...
			setCachedData(
				"extendedInfo",
				JSON.stringify(responseData.extendedInfo),
			);
...
<main class="p-4 mx-auto w-full lg:w-5/6 text-center min-h-full">
	<h1
		class="uppercase text-4xl lg:text-6xl leading-normal font-thin text-svelte"
	>
		EFT Barters
	</h1>
	<h2
		class="text-base lg:text-2xl leading-normal font-thin text-svelte mt-2 italic"
	>
		An easy way to search for barters and hideout crafts in Escape from
		Tarkov
	</h2>
	<br /><br />
	...
	<script
		async
		src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-7853339352311833"
